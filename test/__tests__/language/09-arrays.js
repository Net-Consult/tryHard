describe("Arrays", () => {
  ...
  it("Arrays - 5", () => {
    ...
    var odd = collection.filter(number => number % 2 !== 0);
    expect(odd).toEqual([1, 3, 5, 7, 9]);
  });

  it("Arrays - 6", () => {
    var collection = [1, 2, 3, 4];
    var result = collection.reduce((accumulator, value) => accumulator * value);
    expect(result).toBe(24);
  });

  it("Arrays - 7", () => {
    var collection = [1, 2, 3];
    expect(collection.map(number => number * 2)).toEqual([2, 4, 6]);
  });
