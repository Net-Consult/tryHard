...
/*
const allRoom= hostels
    .reduce((acc,hostels)=>{return acc.concat(hostels.rooms)},[])
    .filter(room=>room.size>=3)
    .sort((hostel1, hostel2)=>{return hostel2.roomName>hostel1.roomName ? -1:1})
;
...
hostels
    ...
    .map(hostel=>{return hostel.rooms
        .map (room=>{return room.roomName = room.roomName.split(' ')
            .map(roomname=>{return roomname.charAt(0).toUpperCase()+ roomname.slice(1)})
            .join(' ')
...
hostels.map(hostel=> hostel.rooms=hostel.rooms
    .filter(room=>room.size<=3));
...
hostels.forEach(hostel => {
    roomNameHostel[hostel.name] = hostel.rooms.filter(room => room.roomName === 'suite marseillaise').length > 0
});
