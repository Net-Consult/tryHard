...
const result1 = hostels
    ...
    .sort((room1, room2 ) => {
        if (room1.roomNumbers < room2.roomNumbers) {
            return -1
        } else{
            return 1
        }
...
// exercice 3 : mettre une majuscule à tous les mots qui sont dans l'attribut RoomNam
...
const result3 = hostels
    ...
    hostels.map(hostel => {
        hostel.rooms = hostel.rooms.filter(room => (room.size<=3)).roomNumbers
        return hostels;
...
const indexOcean = hostels.findIndex (
    hostel => hostel.name ==='hostel ocean');

const hostelOcean = hostels.splice(indexOcean, 1);

hostelOcean.rooms = [];
hostelOcean.roomNumbers = hostelOcean.rooms.length

console.log(indexOcean);
