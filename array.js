...
/*hostels.map((value, index) => value.rooms = value.rooms
    .filter(value => value.size <= 3
    )
)
console.log(hostels);*/
...
[hotelOcean].map(value => value.roomNumbers = value.rooms.length);
...
/*const hostel2 = hostels.forEach(value => value.name === 'hotel ocean');
