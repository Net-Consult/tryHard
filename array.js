...
/*const trie = hostels.sort((hostel1, hostel2) => hostel2.roomNumbers - hostel1.roomNumbers)
...
/*const tab = hostels.reduce((acc, hostel) => acc.concat(hostel.rooms),[])
    .filter(room => room.size >= 3).map(name => {name.roomName; return name}).sort();
...
const [hostel] = hostels.splice(index, 1);
